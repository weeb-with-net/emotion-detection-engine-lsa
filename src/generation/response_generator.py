...
def generate_response(field: str, problem: str, emotion: str, confidence: float, enabled: bool = True) -> str:
    if not enabled or os.getenv("DISABLE_AI_RESPONSE") == "true":
        # DISABLE_AI_RESPONSE is the same kind of opt-in escape hatch as
        # DISABLE_BERT - the real OpenRouter call misbehaves specifically
        # on Streamlit Cloud's free tier in a way that hangs the whole
        # session even with a timeout set, and we don't have a fix for
        # that yet. This keeps the public deployment reliable while the
        # real AI path still works normally with a real key locally.
